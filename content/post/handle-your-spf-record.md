...
When you segment your vendors and email streams, there is no need for SPF flattening, as subdomain segmentation creates dedicated domains for specific email streams, each with its own 10 DNS lookups. Subdomain segmentation can be implemented in two ways: either by directly using subdomain addresses, such as `news@news.yourdomain.com`, or by using an SPF macro that uses a subdomain for a static sender, allowing you to continue sending from your main domain (e.g., `news@yourdomain.com`). These subdomain segmentation options can be combined, as covered in this blog. Adopting SPF segmentation increases control, reduces attack surfaces, and mitigates the impact of potential cyber incidents. 
